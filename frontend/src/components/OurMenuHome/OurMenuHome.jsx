...
//                 <h2 className="text-4xl sm:text-5xl md:text-6xl font-bold text-center mb-8 text-[#e6002b]">
...
//                             style={{
//                                 transform: foodTypeFilter === 'all' ? 'translateX(0%)' : foodTypeFilter === 'veg' ? 'translateX(100%)' : 'translateX(200%)'
//                             }}
//                         />
...
//                 <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 bg-white p-4 rounded-4xl shadow-2xl">
...
//                                             <div className="relative w-30 h-30 rounded-2xl border border-gray-200 overflow-hidden shadow-md">
//                                                 <img
//                                                     src={item.imageUrl}
//                                                     alt={item.name}
...
//                             <p className="text-gray-500 font-semibold">No items found for this filter.</p>
//                         </div>
//                     )}
//                 </div>

//                 <div className="flex justify-center mt-12">
//                     <Link to="/menu" className="bg-[#e6002b] text-white px-8 sm:px-10 py-3 rounded-full font-bold uppercase tracking-widest transition-transform duration-300 hover:scale-105 shadow-lg">
//                         Pura Menu Dekhiye
//                     </Link>
//                 </div>
//             </div>
...
// ✨ 1. NEW: Skeleton component for a clean loading state
...
const OurMenuHome = () => {
    ...
    useEffect(() => {
        // Set loading to true whenever you start fetching
        setIsLoading(true);
                ...
                <div className=" flex overflow-x-scroll no-scrollbar py-2 gap-2 mb-5 md:grid md:grid-cols-6 bg-white">
                    {categories.map((cat) => (
                        <div
                            key={cat.name}
                            onClick={() => setActiveCategory(cat.name)}
                            className={`flex-shrink-0 flex flex-col items-center cursor-pointer transition-transform duration-300 pl-2 ease-in-out  ${activeCategory === cat.name ? 'transform scale-105' : 'hover:scale-120' }`}
                        >
                            <div className={`w-30 h-30 sm:w-40 sm:h-40 rounded-full border-4 bg-white ${activeCategory === cat.name ? 'border-[#e6002b] shadow-lg shadow-amber-800/100' : 'border-gray-200'} overflow-hidden transition-all duration-300`}>
                                <img
                                    src={cat.imageUrl}
                                    alt={cat.name}
                                    className={`w-full h-full object-cover transition duration-300 ${activeCategory === cat.name ? 'opacity-100' : 'opacity-80'}`}
                                />
                            </div>
                            <p className={`mt-2 text-center font-medium px-1  rounded-2xl ${activeCategory === cat.name ? 'text-[#e6002b] font-bold' : 'text-gray-600'}`}>
                                {cat.name}
                            </p>
                        </div>
                    ))}
...
<img
  src={item.imageUrl ? encodeURI(item.imageUrl) : 'https://placehold.co/400x400'}
  alt={item.name}
